...
class BaseBagging(ABC):
    def __init__(self):
        ...
        self.number_of_estimators: int = None
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray, verbose: int = 2):
        ...
        x_samples, y_samples = Bootstrap.create_samples(
            x_train, y_train, self.number_of_estimators, self.max_samples
        )
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        ...
        predictions = [model.predict(x) for model in self.estimator_stack]
